...
def create_masterfullimages_catalogue(args, root_target_fields, root_fullimages_target_fields, telescope_name,
                                      target_field_name, waveband_combinations, psf_image_type_combinations,
    ...
    :param root_fullimages_target_fields:
    ...
    for i in range(len(waveband_combinations)):
        try:
            subprocess.run(['cp', os.path.join(root_fullimages_target_fields, '{}_{}_{}/{}_{}_{}_{}_{}_{}.fits'
                                               .format(psf_image_type_combinations[i],
            ...
            logger.info('Missing {}_{}_{}_{}_{}_{}.fits table'.format(telescope_name, target_field_name,
                                                                      waveband_combinations[i],
                                                                      psf_image_type_combinations[i],
                                                                      sigma_image_type_combinations[i],
                                                                      background_estimate_method_combinations[i]))
    ...
    subprocess.run(['cp', galfit_properties_mastertable_filename, root_fullimages_target_fields])

    multiband_table = combine_properties(galfit_properties_mastertable, wavebands, telescope_name, target_field_name,
                                         args.galaxy_ids_key, args.light_profiles_key, args.galaxy_components_key)

    multiband_table = match_with_source_galaxies_catalogue(multiband_table, source_galaxies_catalogue,
                                                           args.source_galaxies_catalogue_id_key)

    multiband_table_filename = os.path.join(temp_dir, '{}_{}_fullimages_orig.cat'.format(telescope_name,
                                                                                         target_field_name))
    ...
    multiband_table_filename = os.path.join(temp_dir, '{}_{}_fullimages.cat'.format(telescope_name, target_field_name))
    multiband_table.write(multiband_table_filename, format='fits', overwrite=True)

    subprocess.run(['cp', multiband_table_filename, root_target_fields])
    subprocess.run(['cp'] + glob.glob(os.path.join(temp_dir, '*fullimage*.cat')) + [root_fullimages_target_fields])
...
def main(indices, args):
    ...
    for index in indices:
        ...
        root_fullimages_target_fields = grp['root_fullimages_target_fields'][()].decode('utf8')
        ...
        create_masterfullimages_catalogue(args, root_target_fields, root_fullimages_target_fields, telescope_name,
                                          target_field_name, waveband_combinations, psf_image_type_combinations,
                                          sigma_image_type_combinations, background_estimate_method_combinations,
                                          wavebands, source_galaxies_catalogue, temp_dir)
...
def setup(args):
    ...
    description = "Create master catalogue of GALFIT run on images containing target galaxies in Target Fields"
