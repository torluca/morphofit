...
def main(indices, args):
    ...
    root_target_fields = []
    root_regions_target_fields = []
    source_galaxies_catalogues = []
    indices_regions_list = []
    ...
    for target_field_name in target_field_names_list:
        ...
        root_regions_target_fields.append(os.path.join(root_target_field, 'regions').encode('utf8'))
        ...
        indices_regions_list.append([name.encode('utf8') for name in indices_regions])
    ...
    with h5py.File(os.path.join(args.h5pytable_folder, args.h5pytable_filename), mode='w') as h5table:
        for target_field_name in target_field_names_list:
            ...
            grp.create_dataset(name='root_regions_target_fields', data=root_regions_target_fields[idx])
            ...
            grp.create_dataset(name='indices_regions', data=indices_regions_list[idx])
...
def setup(args):
    ...
    description = "Create parameters table to obtain master catalogue of GALFIT run on " \
        "regions containing target galaxies in Target Fields"
