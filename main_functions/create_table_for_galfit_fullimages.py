...
def main(indices, args):
    ...
    root_target_fields, target_field_names, psf_image_types, sigma_image_types, background_estimate_methods, wavebands,\
        sci_image_filenames, rms_image_filenames, seg_image_filenames, \
        exp_image_filenames, exposure_times, magnitude_zeropoints, effective_gains, instrumental_gains, \
    ...
    for target_field_name in target_field_names_list:
        ...
        for j in range(len(combinations)):
            ...
            sci_image_filenames.append(select_images(root_target_field, combinations[j][3], args.sci_images_suffix)
                                       .encode('utf8'))
            seg_image_filenames.append(select_images(root_target_field, combinations[j][3], args.seg_images_suffix)
                                       .encode('utf8'))
            try:
                rms_image_filenames.append(select_images(root_target_field, combinations[j][3], args.rms_images_suffix)
                                           .encode('utf8'))
            ...
            try:
                exp_image_filenames.append(select_images(root_target_field, combinations[j][3], args.exp_images_suffix)
                                           .encode('utf8'))
            except Exception as e:
                print(e)
            ...
            input_galfit_filenames.append('{}_{}_{}_{}_{}_{}.INPUT'.format(args.telescope_name, target_field_name,
                                                                           combinations[j][3], combinations[j][0],
                                                                           combinations[j][1], combinations[j][2])
                                          .encode('utf8'))

            if combinations[j][1] == 'custom_sigma_image':
                sigma_image_filenames.append('{}_{}_{}_sigma_image.fits'.format(args.telescope_name, target_field_name,
                                                                                combinations[j][3]).encode('utf8'))
            else:
                sigma_image_filenames.append('None'.encode('utf8'))

            output_model_image_filenames.append('{}_{}_{}_{}_{}_{}_imgblock.fits'.format(args.telescope_name,
                                                                                         target_field_name,
                                                                                         combinations[j][3],
                                                                                         combinations[j][0],
                                                                                         combinations[j][1],
...
def setup(args):
    ...
    description = "Create parameters table to run GALFIT on images containing target galaxies in Target Fields"
    ...
    parser.add_argument('--source_galaxies_catalogue_suffix', type=str, action='store',
                        default='regions.cat', help='Multiband source galaxies catalogue suffix')
