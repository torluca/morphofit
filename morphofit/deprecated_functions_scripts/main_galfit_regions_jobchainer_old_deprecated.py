...
def run_range(indices):
    ...
    for index in indices:
        ...
        if band in acs_waveband_list:
            ...
            image_name = '{}_{}_{}_{}_drz.fits'.format(additional_text,cluster_name, band,
                                                                              epochs_wfc3[cluster_name])
            seg_image_name = '{}_{}_{}_{}_drz_forced_seg.fits'.format(additional_text,cluster_name, band,
                                                                                             epochs_wfc3[cluster_name])
            rms_image_name = '{}_{}_{}_{}_rms.fits'.format(additional_text,cluster_name, band,
                                                                                  epochs_wfc3[cluster_name])
...
def get_missing(indices):
    ...
    for index in indices:
        ...
        try:
            file = open(root + "{}/regions/pkl_files/x_dict_{}_{}_{}_{}_{}_stamp{}.pkl".format(cluster_name, cluster_name,
                                                                                    band, psf_type,
                                                                                    background_estimate_method,
                                                                                    sigma_image_type, i), 'rb')
            object_file = pickle.load(file)
            file.close()
            file = open(root + "{}/regions/pkl_files/re_dict_{}_{}_{}_{}_{}_stamp{}.pkl".format(cluster_name, cluster_name,
                                                                                     band, psf_type,
                                                                                     background_estimate_method,
                                                                                     sigma_image_type, i), 'rb')
            object_file = pickle.load(file)
            file.close()
            file = open(root + "{}/regions/pkl_files/mag_dict_{}_{}_{}_{}_{}_stamp{}.pkl".format(cluster_name, cluster_name,
                                                                                      band, psf_type,
                                                                                      background_estimate_method,
                                                                                      sigma_image_type, i), 'rb')
            object_file = pickle.load(file)
