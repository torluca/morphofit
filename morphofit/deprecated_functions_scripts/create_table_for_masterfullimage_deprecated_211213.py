...
for target in targets:
    ...
    root_files.append(root.encode('utf8') + '{}/full_image/'.format(target).encode('utf8'))
    output_directories.append(root.encode('utf8') + '{}/full_image/cats/'.format(target).encode('utf8'))
    ...
    combinations = [['{}'.format(x).encode('utf8'), '{}'.format(y).encode('utf8'), '{}'.format(z).encode('utf8'),
                    ...
                    for x, y, z, m in itertools.product(psf_image_types, sigma_image_types,
...
with h5py.File(root + 'h5tables/table_masterfullimage.h5', mode='w') as h5table:
    for target in targets:
