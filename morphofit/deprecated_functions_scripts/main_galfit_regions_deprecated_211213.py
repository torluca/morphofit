...
from morphofit.catalogue_managing import check_parameters_for_next_iteration, assign_sources_to_region
...
def galfit_on_regions(region_index, telescope_name, target_name, waveband, sci_image_region_filename,
                      seg_image_region_filename, exposure_time, magnitude_zeropoint, instrumental_gain, pixel_scale,
                      stamps_mastercatalogue, id_key_sources_catalogue, input_galfit_filename,
                      ...
                      rms_image_region_filename=None,
                      exp_image_region_filename=None,
    ...
    logger.info('=============================== {}, {}, region {}, {}, {}, {}'.format(target_name, waveband,
                                                                                       region_index,
                                                                                       psf_image_type,
                                                                                       sigma_image_type,
    ...
    if check_parameters:
        stamps_mastercatalogue = check_parameters_for_next_iteration(stamps_mastercatalogue, waveband,
    ...
    region_catalogue = assign_sources_to_region(sci_image_region_filename, stamps_mastercatalogue,
                                                ra_keyword='ALPHAWIN_J2000',
                                                dec_keyword='DELTAWIN_J2000', waveband_keyword='f814w')

    logger.info('=============================== create bad pixel mask for GALFIT')
    bad_pixel_mask_filename = create_bad_pixel_mask_for_regions(region_catalogue, seg_image_region_filename,
                                                                id_key_sources_catalogue=id_key_sources_catalogue)

    logger.info('=============================== background estimate in region')
    ...
    light_profiles, source_positions, ra, dec, total_magnitudes, effective_radii, sersic_indices, axis_ratios, \
        ...
        format_properties_for_regions_galfit_single_sersic(sci_image_region_filename, region_catalogue, waveband,
    ...
    create_constraints_file_for_galfit(constraints_file_filename, len(region_catalogue))
    ...
    create_galfit_inputfile(input_galfit_filename, sci_image_region_filename, output_model_image_filename,
    ...
    run_galfit(galfit_binary_file, input_galfit_filename,
               sci_image_region_filename, output_model_image_filename,
...
def main(indices, args):
    ...
    for index in indices:
        ...
        region_index = h5table['region_indices'].value[index].decode('utf8')
        ...
        os.makedirs(root_files + 'regions/region{}_{}_{}_{}'.format(region_index, psf_image_type, sigma_image_type,
        ...
        #                                                        background_estimate_method))
        cwd = root_files + 'regions/region{}_{}_{}_{}/'.format(region_index, psf_image_type, sigma_image_type,
                                                               background_estimate_method)

        sci_image_region_filename = cwd + h5table['sci_image_region_filenames'].value[index].decode('utf8')
        rms_image_region_filename = cwd + h5table['rms_image_region_filenames'].value[index].decode('utf8')
        seg_image_region_filename = cwd + h5table['seg_image_region_filenames'].value[index].decode('utf8')
        exp_image_region_filename = cwd + h5table['exp_image_region_filenames'].value[index].decode('utf8')
        stamps_mastercatalogue_filename = cwd + h5table['stamps_mastercatalogue_filenames'].value[index].decode('utf8')
        ...
        os.system('ln -sf {}regions/{} {}'.format(root_files, os.path.basename(sci_image_region_filename), cwd))
        os.system('ln -sf {}regions/{} {}'.format(root_files, os.path.basename(seg_image_region_filename), cwd))
        os.system('ln -sf {}stamps/cats/{} {}'.format(root_files, os.path.basename(stamps_mastercatalogue_filename),
                                                      cwd))
        os.system('ln -sf {}stars/{} {}'.format(root_files, os.path.basename(psf_image_filename), cwd))

        if os.path.basename(rms_image_region_filename) == 'None':
            rms_image_region_filename = None
        else:
            os.system('ln -sf {}regions/{} {}'.format(root_files, os.path.basename(rms_image_region_filename), cwd))
        if os.path.basename(exp_image_region_filename) == 'None':
            exp_image_region_filename = None
        else:
            os.system('ln -sf {}regions/{} {}'.format(root_files, os.path.basename(exp_image_region_filename), cwd))

        stamps_mastercatalogue = Table.read(stamps_mastercatalogue_filename, format='fits', memmap=True)
        out_dir = root_files + 'regions/region{}_{}_{}_{}/'.format(region_index, psf_image_type,
                                                                   sigma_image_type, background_estimate_method)
        best_fit_properties_h5table_filename = out_dir + '{}_{}_{}_{}_{}_{}_{}.h5'.format(telescope_name, target_name,
                                                                                          waveband, region_index,
                                                                                          psf_image_type,
                                                                                          sigma_image_type,
                                                                                          background_estimate_method)

        galfit_on_regions(region_index, telescope_name, target_name, waveband, sci_image_region_filename,
                          seg_image_region_filename, exposure_time, magnitude_zeropoint, instrumental_gain, pixel_scale,
                          stamps_mastercatalogue, id_key_sources_catalogue, input_galfit_filename,
                          sigma_image_filename, sigma_image_type, background_value,
                          background_estimate_method, output_model_image_filename, psf_image_type, psf_image_filename,
                          psf_sampling_factor, convolution_box_size, galfit_binary_file,
                          best_fit_properties_h5table_filename,
                          rms_image_region_filename=rms_image_region_filename,
                          exp_image_region_filename=exp_image_region_filename,
...
def check_missing(indices, args):
    ...
    for index in indices:
        ...
        best_fit_properties_imgblock_filename = cwd + \
            '{}_{}_{}_region{}_{}_{}_{}_imgblock.fits'.format(telescope_name,
