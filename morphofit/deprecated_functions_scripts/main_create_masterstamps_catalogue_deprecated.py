...
# morphofit imports
from morphofit.catalogue_managing import get_median_properties, create_fixed_image_table, add_dictionary_item
from morphofit.catalogue_managing import match_with_target_galaxies_catalogue, delete_repeating_sources
...
def create_masterstamps_catalogue(root_files, output_directory, telescope_name, target_name, waveband_combinations,
                                  indices_target_galaxies, stamp_index_combinations, psf_image_type_combinations,
    ...
    for stamp_number in indices_target_galaxies:
        try:
            number_galaxies_stamp = max([len(v) for k, v in mag_dictionary.items() if (target_name in k) &
            ...
            table.write(output_directory + '{}_{}_stamp{}_multiband.forced.sexcat'
                        .format(telescope_name, target_name, stamp_number), format='fits', overwrite=True)
            multiband_tables.append(table)
        except Exception as exception:
            logger.info(exception)
            logger.info('Missing {}'.format(stamp_number))
            pass
    ...
    multiband_table.write(output_directory + '{}_{}_stamps_multiband.forced.sexcat'
                          .format(telescope_name, target_name),
...
def main(indices, args):
    ...
    for index in indices:
        ...
        indices_target_galaxies = grp['indices_target_galaxies'].value
        stamp_index_combinations = grp['stamp_index_combinations'].value
        ...
        background_estimate_method_combinations = [name.decode('utf8')
                                                   for name in
                                                   grp['background_estimate_method_combinations'].value]
        ...
        create_masterstamps_catalogue(root_files, output_directory, telescope_name, target_name, waveband_combinations,
                                      indices_target_galaxies, stamp_index_combinations, psf_image_type_combinations,
                                      sigma_image_type_combinations, background_estimate_method_combinations,
                                      wavebands, psf_image_types, sigma_image_types, background_estimate_methods,
                                      target_galaxies_catalogue, full_galaxies_catalogue)
...
def check_missing(indices, args):
    ...
    for index in indices:
        ...
        try:
            table = Table.read(output_directory + '{}_{}_stamps_multiband.forced.sexcat'
...
def setup(args):
    ...
    description = "Create master catalogue from GALFIT on stamps"
