...
psf_image_types = ['pca_psf', 'observed_psf', 'moffat_psf',
                   'effective_psf']
...
for target in targets:
    ...
    root_files.append(root.encode('utf8') + '{}/stamps/'.format(target).encode('utf8'))
    output_directories.append(root.encode('utf8') + '{}/stamps/cats/'.format(target).encode('utf8'))
    target_galaxies_catalogue = Table.read(root + '{}/{}_{}_target_multiband.forced.sexcat'
                                           .format(target, telescope_name, target), format='fits')
    ...
    indices_target_galaxies.append(list(np.arange(0, n_target_galaxies, 1, dtype=int)))
    combinations_length = len(psf_image_types) * len(background_estimate_methods) * len(sigma_image_types) * \
        len(wavebands) * n_target_galaxies
    combinations = [['{}'.format(x).encode('utf8'), '{}'.format(y).encode('utf8'), '{}'.format(z).encode('utf8'),
                    ...
                    for x, y, z, m, n in itertools.product(psf_image_types, sigma_image_types,
                                                           ...
                                                           np.arange(0, n_target_galaxies, 1, dtype=int))]
...
with h5py.File(root + 'h5tables/table_masterstamps.h5', mode='w') as h5table:
    for target in targets:
        ...
        grp.create_dataset(name='target_galaxies_catalogue_filenames',
                           data=target_galaxies_catalogue_filenames[idx])
